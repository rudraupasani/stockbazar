...
import { ArrowDownRight, TrendingDown, ExternalLink, BarChart3 } from "lucide-react";
...
const TopLosers = () => {
    ...
    useEffect(() => {
        async function fetchLosers() {
            try {
                const res = await axios.get("https://stockbazar-4.onrender.com/api/market/top-movers");
    ...
    useEffect(() => {
        ...
        ws.onmessage = (msg) => {
            try {
                ...
                console.error("Error parsing WebSocket message in TopLosers:", err);
            }
        };
        return () => ws.close();
    }, []);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {losers.map((coin, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            whileHover={{ y: -3 }}
                            className="sb-card p-5 flex items-center gap-4 cursor-pointer"
