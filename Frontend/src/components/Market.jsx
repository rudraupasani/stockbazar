...
const MarketPage = () => {
    ...
    useEffect(() => {
        wsRef.current = new WebSocket("import.meta.env.VITE_WS_BASE_URL/ws/live-prices");
        ...
        >
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
                ...
                >
                    ...
                    <div>
                        {coins.map((coin, i) => (
                            <div
                                key={`${coin.symbol}-${i}`}
                                className="grid grid-cols-8 gap-4 px-6 py-4 border-b transition-colors duration-150 cursor-pointer"
                ...
                <div className="lg:hidden grid gap-4">
                    {coins.map((coin, i) => (
                        <div key={`m-${coin.symbol}-${i}`} className="sb-card p-5 cursor-pointer" onClick={() => navigate(`/coin/${coin.symbol?.toLowerCase()}`)}>
