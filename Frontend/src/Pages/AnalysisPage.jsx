...
const signalColor = (s = "") => {
    if (!s || typeof s !== "string") return { color: "var(--accent)", bg: "var(--accent-bg)" };
...
const riskColor = (r = "") => {
    if (!r || typeof r !== "string") return "var(--accent)";
    if (r.trim() === "Low") return "var(--green)";
    if (r.trim() === "Medium") return "var(--accent)";
    return "var(--red)";
};
...
const normalizeAnalysis = (analysis) => {
    ...
    return {
        summary_beginner: str(analysis.summary_beginner, "No summary available."),
        summary_advanced: str(analysis.summary_advanced, "No technical summary available."),
        trend: str(analysis.trend, "Neutral"),
        trend_score: clamp(analysis.trend_score),
        momentum: str(analysis.momentum, "Neutral"),
        volatility: str(analysis.volatility, "Low"),
        support: str(analysis.support, "N/A"),
        resistance: str(analysis.resistance, "N/A"),
        stop_loss: str(analysis.stop_loss, "N/A"),
        take_profit: str(analysis.take_profit, "N/A"),
        sentiment: str(analysis.sentiment, "Neutral"),
        whale_activity: str(analysis.whale_activity, "Neutral"),
        pattern: str(analysis.pattern, "None"),
        prediction_24h: str(analysis.prediction_24h, "N/A"),
        prediction_7d: str(analysis.prediction_7d, "N/A"),
        risk: str(analysis.risk, "Medium"),
        confidence: clamp(analysis.confidence),
        grade: str(analysis.grade, "C"),
        buy_signal: str(analysis.buy_signal, "Hold"),
        key_levels: Array.isArray(analysis.key_levels)
            ? analysis.key_levels.map((l) => (typeof l === "string" ? l.trim() : String(l))).filter(Boolean)
            : [],
    };
};
...
const PriceLevelChart = ({ levels, currentPrice }) => {
    const parsedLevels = levels
        .map((level) => {
            const cleaned = String(level.value || "").replace(/[^0-9.]/g, "");
            if (!cleaned) return null;
            const num = Number(cleaned);
            return Number.isFinite(num) && num > 0 ? { ...level, numeric: num } : null;
        })
        .filter(Boolean);
...
const AnalysisPage = () => {
    ...
    /* Fetch coin list */
    useEffect(() => {
        axios.get("https://stockbazar-4.onrender.com/api/market/top-gainers")
            .then((r) => {
    ...
    const runAnalysis = async () => {
        ...
        setError("");
        try {
            const r = await axios.post("https://stockbazar-4.onrender.com/api/ai/analysis", {
                coinData: {
